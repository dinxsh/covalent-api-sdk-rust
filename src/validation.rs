...
impl Validator {
    ...
    ///
    /// Supports both v1 (`ckey_` prefix + 27 hex chars) and v2 (`cqt_` prefix + 26 alphanumeric chars) formats.
    ...
    pub fn validate_api_key(api_key: &str) -> Result<()> {
        ...
        // Validate known key formats
        if let Some(suffix) = api_key.strip_prefix("ckey_") {
            // v1 format: ckey_ + 27 hex chars = 32 total
            if suffix.len() != 27 {
                return Err(Error::Config(format!(
                    "ckey_ API key must have 27 characters after prefix, got {}", suffix.len()
                )));
            }
            if !suffix.chars().all(|c| c.is_ascii_hexdigit()) {
                return Err(Error::Config("ckey_ API key contains invalid hex characters".to_string()));
            }
        } else if let Some(suffix) = api_key.strip_prefix("cqt_") {
            // v2 format: cqt_ + 26 alphanumeric chars = 30 total
            if suffix.len() != 26 {
                return Err(Error::Config(format!(
                    "cqt_ API key must have 26 characters after prefix, got {}", suffix.len()
                )));
            }
            if !suffix.chars().all(|c| c.is_alphanumeric()) {
                return Err(Error::Config("cqt_ API key contains invalid characters".to_string()));
            }
        }
        // Allow other formats for forward compatibility
    ...
    pub fn validate_nft_identifier(contract_address: &str, token_id: &str) -> Result<()> {
        ...
        } else if token_id.parse::<u64>().is_err() {
...
mod tests {
    ...
    #[test]
    fn test_api_key_validation_ckey() {
        // Valid ckey format: ckey_ + 27 hex chars
        assert!(Validator::validate_api_key("ckey_abcdef1234567890abcdef12345").is_ok());
    }

    #[test]
    fn test_api_key_validation_cqt() {
        // Valid cqt format: cqt_ + 26 alphanumeric chars
        assert!(Validator::validate_api_key("cqt_abcdefghij1234567890abcdef").is_ok());
    }

    #[test]
    fn test_api_key_validation_placeholder() {
        assert!(Validator::validate_api_key("your-api-key-here").is_err());
        assert!(Validator::validate_api_key("test-key-1234567").is_err());
    }
